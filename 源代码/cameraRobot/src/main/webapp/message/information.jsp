	...
	<script type="text/javascript" src="http://libs.baidu.com/jquery/1.11.1/jquery.min.js"></script>
   	<script type="text/javascript">
		...
		$(function ajaxSend(){
			$.ajax({
				type : "post",
				url : "<%=path %>/user/findAllProvince.action",
				dataType : "json",
				error : function(err) {
					alert("加载所有省份未成功");
					alert(err.code);
		...
		function ajaxSendCallBack(data) {
			for ( var i = 0; i < data.length; i++) {
				...
				op.value = data[i].areaId;
				var textNode = document.createTextNode(data[i].areaName);
		...
		function myCity(){
			...
			$.ajax({
				...
				url : "<%=path %>/user/findAllCityByProvincename.action",
				dataType : "json",
				error : function(err) {
					alert(err.code);
				},
		...
		function myCounty(){
			...
			$.ajax({
				...
				url : "<%=path %>/user/findAllCountyBycityname.action",
				dataType : "json",
				error : function(err) {
					alert(err.code);
				},
				success : ajaxSendCallBackCounty
			})
		};
		
		function ajaxSendCallBackCounty(data) {
			var countySelect = document.getElementById("county");
			var optionEleList = countySelect.getElementsByTagName("option");
			
			while (optionEleList.length > 1) {
				countySelect.removeChild(optionEleList[1]);
			}
			...
			for ( var i = 0; i < data.length; i++) {
				...
				document.getElementById("county").appendChild(op);
			}
		}
		function inputAddress(){
			...
			$("#addr").attr("value",province+city+county);
			//alert($("#addr").val());
		}
   	</script>
                ...
                <div class="col-lg-12">
                  <div class="card">
                    ...
                    <div class="card-body">
                      ...
                      <form action="<%=path %>/user/updateUser.action" method="post" enctype="multipart/form-data" class="form-horizontal">
                      	<!--1.用户名-->
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <select id="sex" name="sex" data-placeholder="选择类别" class="form-control mb-3">
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <input type="text" id="name" name="name" value="${user.name}" class="form-control"><small class="help-block-none"></small>
                          </div>
                        </div>
                        <div class="line"></div>
                        <!--4.头像-->
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <input id="file" type="file" name="file" value="${user.img}" class="form-control-file" style="width:150px; display:inline;">
							<img alt="...未选择" style="height: 70px;width:70px; background-color: gray; display:inline;" id="image"  src="/userImg/${user.img }">
                          </div>
                        </div>
                        <div class="line"></div>
                        <!--5.微信-->
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <input type="text" id="wechat" name="wechat" value="${user.wechat}" class="form-control">
                          </div>
                        </div>
                        <div class="line"></div>
                        <!--6.Email-->
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-sm-9">	
                         	<input type="text" name="addr" id="addr" value="${user.addr}" class="form-control"/>
                            <select id="province" name="province" onchange="myCity()" data-placeholder="选择类别" class="form-control mb-3" style="width:150px; display:inline;">
                              <option value="0">-请选择-</option>
                            </select>
                            <label class="col-sm-3 form-control-label" style="width:20px;">省</label>
                            <select id="city" name="city" onchange ="myCounty()" data-placeholder="选择类别" class="form-control mb-3" style="width:150px; display:inline;">
                              <option value="0">-请选择-</option>
                            </select>
                            <label class="col-sm-3 form-control-label" style="width:20px;">市</label>
                            <select id="county" name="county" onchange ="inputAddress()" data-placeholder="选择类别" class="form-control mb-3" style="width:150px; display:inline;">
                              <option value="0">-请选择-</option>
                            </select>
                            <label class="col-sm-3 form-control-label" style="width:20px;">县</label>
                        ...
                        <div class="form-group row">
                          <div class="col-sm-4 offset-sm-3">
                            <button type="button" onclick="javascrtpt:window.location.href='<%=path %>/signal/index.action?userId=${user.id}'" class="btn btn-secondary">取消</button>
